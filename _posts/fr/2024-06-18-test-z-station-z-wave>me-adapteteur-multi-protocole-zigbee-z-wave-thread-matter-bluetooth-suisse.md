...
Z-Station prend en charge Z-Wave et Zigbee NCP, OpenThread RCP, Bluetooth Low Energy ou Zigbee NCP/OpenThread RCP (au choix, sélectionné par mise à jour du firmware). Il est également possible d'utiliser la Z-Station comme répéteur Z-Wave/Zigbee/Thread (micrologiciel spécial requis).

### Prise en charge de Z-Wave

Comparé à la conception standard du micrologiciel utilisée par presque toutes les clés USB Z-Wave et autres matériels d'interface hôte Z-Wave, **le micrologiciel de cet appareil offre plusieurs extensions et améliorations:**
...
N- otification de réveil étendue pour prolonger la durée de vie de la batterie des appareils fonctionnant sur batterie dans le réseau
...
La prise en charge de **Matter nécessite le micrologiciel Thread** et une application hôte prenant en charge le protocole Matter. L'adaptateur Z-Station avec le micrologiciel OpenThread est compatible avec le code source du projet CHIP.


### Prise en charge Bluetooth basse consommation (BLE)

Le micrologiciel BLE RCP fournit une interface série HCI. Pris en charge par **l'infrastructure Bluetooth Linux BlueZ**.

## Mise à jour du Firmware

### Z-wave

Rien de plus simple pour faire la mise à jour du firmware Z-wave, la société z-wave me à pensé à tout et à créé un outil afin de te faciliter la tâche. Clic sur le lien ci-dessous, branche l'adaptateur {{ page.ref }} à ton pc et clic sur **start**. Sélectionne bien la bonne puce sous linux ```dev/ttyUSB0``` et en bas du résultat d'analyse tu trouveras l'onglet **update info** qui te permettras si une mise à jour est dispo de mettre à jour le firmware et le bootloader.
...
Pour le micrologiciel OpenThread : ```zme_open_thread info -d /dev/ttyXX```
...
Pour mettre à niveau le micrologiciel, procédez : ```zme_zigbee update zigbee_EFR32MG21A020F1024IM32.gbl -d /dev/ttyXX```. Utilisez uniquement les firmwares Z-Wave.Me. Utilisez zme_zigbee, zme_open_thread ou zme_ble selon le firmware que vous avez actuellement sur l'appareil.

Habituellement, il suffit d'exécuter la mise à niveau. Mais dans certains cas, vous devrez peut-être également mettre à jour le micrologiciel SE et Bootloader. Pour cela faites :
...
[Page principale de Z-way](https://z-wave.me/z-way/){: target="_blank"}
[Serveur de control z-way](https://z-wave.me/z-way/download-z-way/){: target="_blank"}

[Intégration du serveur z-way dans HAOS](https://z-wave.me/z-way/integrations/#HomeAssistant){: target="_blank"}

[Page d'aide zwave.me](https://help.z-wave.me/){: target="_blank"}
...
- interface UART supplémentaire (nécessite une version personnalisée micrologiciel),
...
Z-Station peut également fonctionner avec le [micrologiciel Z-Uno](https://z-uno.z-wave.me){: target="_blank"} et piloter des broches GPIO.
